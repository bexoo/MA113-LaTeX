...
If we have non-overlapping regions $R = R_1 \cup R_2 \ni R_1 \cap R_2 = \varnothing$ then
...
\end{pmatrix}. \]
Differentiating this gives us the Jacobian matrix $\mathbf{J}$ of $r(\theta)$
\[ \begin{bmatrix}
    \cos{\theta} & -r\sin{\theta} \\
    \sin{\theta} & r\cos{\theta}
\end{bmatrix}. \]
...
The Jacobian matrix measures the amount of ``stretching'' for each point in some coordinate space $(x,y)$ to another coordinate space $(u,v)$. This is useful to us because in two dimensions, the determinant of the Jacobian is $\frac{\dd{u}\dd{v}}{\dd{x}\dd{y}}$.
